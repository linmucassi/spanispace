...
0b. `supabase/add-application-journeys.sql` — backs the new internal-vs-external application journeys (see the "Application journeys" row in [What's Built](#whats-built) above, and `docs/CHANGELOG/22 AUG 2026 - Application journeys, internal vs external.md`): `jobs.origin`/`apply_mode` columns and the `university_application_interests` table. Not urgent like 0 above (nothing is broken without it — every write path degrades gracefully to the old columns/behavior), but none of the new redirect-capture, source-filtering, or university-interest-tracking features are actually active until it's run.
0c. `supabase/add-talent-sourcing-and-verification.sql` — backs Direct Talent Sourcing and Candidate Document Verification (see the "Talent sourcing & verification" row in [What's Built](#whats-built) above, and `docs/CHANGELOG/22 AUG 2026 - Direct talent sourcing and candidate document verification.md`): `candidate_profiles.open_to_offers`, the new `job_invites` table, and `candidate_documents.verification_status`/new doc types. Also not urgent — every new write path falls back gracefully — but the talent pool, invites, and document verification are all inert until it's run.
