...
| Google OAuth | 🟢 Live for candidates | See [1.3](#13-social-authentication--shipped-for-candidates-companies-still-emailpassword). Companies still email/password by design. **14 Aug 2026: switched off Supabase's hosted redirect flow** — Google's consent screen was showing the raw `rssuacaedvihhpcakuvm.supabase.co` project domain instead of spanispace.com, which read as a phishing attempt. Now uses Google Identity Services + `signInWithIdToken`, entirely client-side on spanispace.com. |
...
- **Fixed 14 Aug 2026: the daily expiry-alerts/profile-nudges run was failing every day.** `notifications.yml`'s daily cron (`0 5 * * *`) and hourly cron (`0 * * * *`) both fired at the exact same instant once a day; GitHub queued two runs of the workflow back to back, and the later one (the daily-flagged run) intermittently started with an empty secrets context, failing with "Missing SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY." Confirmed via `gh run list`/`gh run view` against the live Actions history — 6/6 logged failures over 9 days, all in the 05:00-07:40 UTC window right after that hour's hourly run, every other hourly run (same secrets) succeeding without exception. Fixed by offsetting the daily/weekly crons to `:20` past the hour so they no longer collide with the hourly trigger's dispatch instant. Reported by Linda as "failing every day" — likely via GitHub's automatic email to the repo owner on scheduled-workflow failure.
