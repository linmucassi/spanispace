          ...
          <thead>
            <tr className="bg-slate-50 text-left">
              {['Role', 'Intern', 'Junior', 'Mid', 'Senior', 'Lead'].map((head) => (
                <th
                  key={head}
                  className="px-4 py-3.5 font-mono text-[11px] uppercase tracking-wider text-slate-400 font-bold border-b border-slate-200"
                >
                  {head}
                </th>
              ))}
          ...
          <tbody>
            {academy.salaries.map((row) => (
              <tr key={row.role} className="border-b border-slate-100 last:border-0">
                <th scope="row" className="px-4 py-3.5 font-semibold text-slate-900 text-left">
                  <span
                    className={`inline-block w-2.5 h-2.5 rounded-full mr-2.5 align-middle ${
                      TRACK_DOT[row.track] || 'bg-slate-400'
                    }`}
                  />
                  {row.role}
                </th>
                <td className="px-4 py-3.5 text-slate-600 tabular-nums whitespace-nowrap">{row.intern}</td>
                <td className="px-4 py-3.5 text-slate-600 tabular-nums whitespace-nowrap">{row.junior}</td>
                <td className="px-4 py-3.5 text-slate-600 tabular-nums whitespace-nowrap">{row.mid}</td>
                <td className="px-4 py-3.5 text-slate-600 tabular-nums whitespace-nowrap">{row.senior}</td>
                <td className="px-4 py-3.5 text-slate-600 tabular-nums whitespace-nowrap">{row.lead}</td>
              </tr>
            ))}
          </tbody>
        </table>
