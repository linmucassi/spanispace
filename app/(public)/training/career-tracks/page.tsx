    ...
    >
      <div className="space-y-3">
        {academy.tracks.map((track) => (
          <details
            key={track.name}
            className="group bg-white border border-ink-200 rounded-2xl overflow-hidden"
          >
            <summary className="flex items-center gap-4 p-5 cursor-pointer list-none [&::-webkit-details-marker]:hidden">
              <span className="flex-1 font-bold text-ink-900 text-lg">{track.name}</span>
              <span className="text-sm text-ink-400 whitespace-nowrap">
                <T k={track.roles.length === 1 ? 'course.role' : 'course.roles'} vars={{ n: track.roles.length }} />
              </span>
              <span className="text-ink-300 text-2xl leading-none transition-transform group-open:rotate-45">
                +
              </span>
            </summary>
            <div className="px-5 pb-6 border-t border-ink-100">
              <p className="text-ink-600 my-5">
                <Inline html={track.summaryHtml} />
              </p>
              <div className="space-y-3">
                {track.roles.map((role) => (
                  <div key={role.title} className="bg-ink-50 border border-ink-200 rounded-xl p-5">
                    <h2 className="font-bold text-ink-900 text-lg mb-2">{role.title}</h2>
                    <p className="text-sm text-ink-600 mb-2.5">
                    ...
                    <div className="grid sm:grid-cols-2 gap-4 mb-3">
                      <div>
                        <p className="text-[10px] font-mono font-bold uppercase tracking-wider text-ink-400 mb-1.5">
                          Core skills
                        </p>
                        <div className="flex flex-wrap gap-1.5">
                          {role.coreSkills.map((skill) => (
                            <span
                              key={skill}
                              className="text-xs px-2.5 py-1 rounded-full bg-white border border-ink-200 text-ink-600"
                            >
                              {skill}
                            </span>
                          ))}
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {role.starterCerts.map((cert) => (
                              <span
                                key={cert}
                                className="text-xs px-2.5 py-1 rounded-full bg-white border border-brand-200 text-brand-600"
                              >
                                {cert}
                              </span>
                            ))}
